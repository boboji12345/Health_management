...
<head>
    ...
    <script type="text/javascript">
        ...
        function deleteById(aid){
            if(confirm("是否删除"))
                window.location="${ctx}/admin/deleteUserRholidaysInfoById?Id="+aid;
        }
        ...
        function deleteAll(){
            if(confirm("确定要批量删除吗？")){
                ...
                window.location="${ctx}/admin/deleteUserRholidaysInfoById2?Id="+endids;
            }
        }
        ...
        function updateInfo(aid){
            ...
            $.ajax({
                url:"${ctx}/admin/getRholidaysInfoById",
                type:"get",
                data:{id:aid},
                dataType:"json",
                success:function(data){
                    $("#uuname").val(data.obj["hcont"]);
                }
            });
        }

        function update() {
            var rntitle = $("#uuname").val();
            var id = $("#userid").val();

            if(isEmpty(rntitle)){
                alert("休假理由信息不能为空，请确认一下！");
                return;
            }

            $.ajax({
                url:"${ctx}/admin/upUserHolidaysInfo",
                type:"get",
                data : {
                    id: id,
                    rcont: rntitle
        ...
        function save() {
            ...
            $.ajax({
                url:"${ctx}/admin/addUserHolidaysInfo",
                type:"get",
                data : {
                    rcont: rntitle
                },
                ...
                <nav class="sidebar-main">
                    <ul class="nav nav-drawer">
                        ...
                        <li class="nav-item nav-item-has-subnav active open">
                            ...
                            <ul class="nav nav-subnav">
                                <li class="active"> <a href='${ctx}/admin/userrholidaysmana'>学生休假管理</a> </li>
                                ...
                                <nav aria-label="Page navigation example">
                                    <ul class="pagination">
                                        ...
                                        <@fpage.fpage page=admininfo.page pagesize=admininfo.pagesize totalpages=admininfo.totalpages totalrecords=admininfo.totalrecords
                                            url="userrholidaysmana?key=${Request.selectName}" />
                                    </ul>
                                </nav>
...
<#include "adminholidays_add.ftl">
<#include "adminholidays_update2.ftl">
</body>
</html>
