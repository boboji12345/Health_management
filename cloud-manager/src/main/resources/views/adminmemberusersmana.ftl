...
<head>
    ...
    <script type="text/javascript">
        ...
        function deleteById(aid){
            if(confirm("是否删除"))
                window.location="${ctx}/admin/deleteUsersInfoById?Id="+aid;
        }
        ...
        function deleteAll(){
            if(confirm("确定要批量删除吗？")){
                ...
                window.location="${ctx}/admin/deleteUsersInfoById?Id="+endids;
            }
        }

        function exportstus(){
            if(confirm("是否要导出到Excel吗"))
                window.location="${ctx}/admin/exportusers";
        }

        function inportstus(){
            if(confirm("是否要导入Excel数据吗"))
                window.location="${ctx}/admin/importusers";
        }

        function selectClick(){
            var name = encodeURI(encodeURI($("#inputName").val()));
            window.location="${ctx}/admin/selectUsersInfoByFuzzyquery?name="+name;
        }
        ...
        function updateInfo(aid){
            ...
            $.ajax({
                ...
                success:function(data){
                    ...
                    $("#uUstuids").val(parseInt(data.obj["ustuids"]));
                    $("#uUshen").val(data.obj["ushen"]);
        ...
        function update() {
            ...
            var ustuids = $("#uUstuids").val();
            var ushen = $("#uUshen").val();
            var bname = $("#selectdatas").val();

            if(isEmpty(upwd) || isEmpty(utel) || isEmpty(uname) || isEmpty(uroomnumber) || isEmpty(ustuids) || isEmpty(ushen)
                || isEmpty(uage)){
                alert("学生用户信息不能为空，请确认一下！");
                return;
            }
                    ...
                    <ul class="nav nav-drawer">
                        ...
                        <li class="nav-item nav-item-has-subnav active open">
                            ...
                            <ul class="nav nav-subnav">
                                ...
                                <li class="active"> <a href='${ctx}/admin/adminmemberusersmana'>学生信息管理</a> </li>
            ...
            <nav class="navbar navbar-default">
                <div class="topbar">
                    <div class="topbar-left">
                        ...
                        <span class="navbar-page-title"> 首页 —— 学生用户信息管理 </span>
                    </div>
                        ...
                        <div class="card">
                            <div class="card-toolbar clearfix">
                                ...
                                <div class="toolbar-btn-action">
                                    ...
                                    <a class="btn btn-danger" href="#!" onclick="deleteAll()">
                                        <i class="mdi mdi-window-close"></i>批量删除</a>
                                    <a  class="btn btn-success" href="javascript:void(0)"
                                        onclick="inportstus()" >一键导入</a>
                                    <a  class="btn btn-success" href="javascript:void(0)"
                                        onclick="exportstus()" >一键导出</a>
                                </div>
                            </div>
                            <div class="card-body">
                                ...
                                <nav aria-label="Page navigation example">
                                    <ul class="pagination">
                                        ...
                                        <@fpage.fpage page=memberusers.page pagesize=memberusers.pagesize totalpages=memberusers.totalpages totalrecords=memberusers.totalrecords
                                            url="adminmemberusersmana?key=${Request.selectName}" />
                                    </ul>
                                </nav>
...
<#include "adminuser_add.ftl">
<#include "adminuser_update.ftl">
