...
<head>
    ...
    <script type="text/javascript">
        ...
        function deleteById(aid){
            if(confirm("是否删除"))
                window.location="${ctx}/admin/deleteUserRdailyclocksInfoById?Id="+aid;
        }
        ...
        function deleteAll(){
            if(confirm("确定要批量删除吗？")){
                ...
                window.location="${ctx}/admin/deleteUserRdailyclocksInfoById2?Id="+endids;
            }
        }

        function selectClick(){
            var name = encodeURI(encodeURI($("#inputName").val()));
            window.location="selectUserRdailyclocksInfo?name="+name;
        }
        ...
        function updateInfo(aid){
            ...
            $.ajax({
                url:"${ctx}/admin/getRdailyclocksInfoById",
                type:"get",
                data:{id:aid},
                dataType:"json",
                success:function(data){
                    $("#upattr").val(data.obj["pattr"]);
                    $("#uptemperature").val(data.obj["ptemperature"]);
                    $("#upuzhengjianhaoma").val(data.obj["puzhengjianhaoma"]);
                    $("#uplaiyuan").val(data.obj["plaiyuan"]);

                    $("input[name='uzhuangkuang']").each(function(){
                        if( $.trim($(this).val()) === $.trim(data.obj["pshentizhuangkuang"]) ){
        ...
        function update() {
            var pattr = $("#upattr").val();
            var ptemperature = $("#uptemperature").val();
            var id = $("#userid").val();
            var puzhengjianhaoma = $("#upuzhengjianhaoma").val();
            var plaiyuan = $("#uplaiyuan").val();
            var pshentizhuangkuang = $("input[name='uzhuangkuang']:checked").val();

            if(isEmpty(pattr)||isEmpty(ptemperature)||isEmpty(puzhengjianhaoma)||isEmpty(plaiyuan)||isEmpty(pshentizhuangkuang)){
                alert("健康打卡信息不能为空，请确认一下！");
                return;
            }

            $.ajax({
                url:"${ctx}/admin/upAdminDailyclocksInfo",
                type:"get",
                data : {
                    id: id,
                    pattr: pattr,
                    ptemperature: ptemperature,
                    puzhengjianhaoma: puzhengjianhaoma,
                    plaiyuan: plaiyuan,
                    pshentizhuangkuang: pshentizhuangkuang
                },
        ...
        function save() {
            var pattr = $("#apattr").val();
            var ptemperature = $("#aptemperature").val();
            var puzhengjianhaoma = $("#apuzhengjianhaoma").val();
            var plaiyuan = $("#aplaiyuan").val();
            var pshentizhuangkuang = $("input[name='azhuangkuang']:checked").val();
            ...
            $.ajax({
                url:"${ctx}/admin/addAdminDailyclocksInfo",
                type:"get",
                data : {
                    pattr: pattr,
                    ptemperature: ptemperature,
                    ...
                    <ul class="nav nav-drawer">
                        ...
                        <li class="nav-item nav-item-has-subnav active open">
                            ...
                            <ul class="nav nav-subnav">
                                ...
                                <li class="active"> <a href='${ctx}/admin/userrdailyclocksmana'>健康打卡管理</a> </li>
            ...
            <nav class="navbar navbar-default">
                <div class="topbar">
                    <div class="topbar-left">
                        ...
                        <span class="navbar-page-title"> 首页 —— 健康打卡管理 </span>
                    </div>
                                ...
                                <nav aria-label="Page navigation example">
                                    <ul class="pagination">
                                        ...
                                        <@fpage.fpage page=admininfo.page pagesize=admininfo.pagesize totalpages=admininfo.totalpages totalrecords=admininfo.totalrecords
                                            url="userrdailyclocksmana?key=${Request.selectName}" />
                                    </ul>
                                </nav>
...
<#include "admindailyclocks_add.ftl">
<#include "admindailyclocks_update.ftl">
